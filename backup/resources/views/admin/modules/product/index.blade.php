                ...
                <div class="table-responsive">
                    <table id="multi_col_order" class="table table-bordered display"
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$count++}}</td>
                                <td><img src="{{ asset('product_images/' . $product->main_image) }}" style="width:50px;" alt=""></td>
                                <td>{{$product->title}}</td>
                                <td>&pound;{{$product->price}}</td>
                                <td>
                                    @if($product->is_league_team)
                                        <div class="mb-1">
                                            <span class="badge rounded-pill font-weight-medium bg-light-secondary text-secondary">League Team</span>
                                        </div>
                                        <div class="mb-1">
                                            <span class="badge rounded-pill font-weight-medium bg-light-primary text-primary">{{$product->league->title}}</span>
                                        </div>
                                        <div class="mb-1">
                                            <span class="badge rounded-pill font-weight-medium bg-light-primary text-warning">{{$product->league_team->name}}</span>
                                        </div>
                                    @elseif($product->is_country_team)
                                        <div class="mb-1">
                                            <span class="badge rounded-pill font-weight-medium bg-light-primary text-primary">Country Team</span>
                                        </div>
                                        <div class="mb-1">
                                            <span class="badge rounded-pill font-weight-medium bg-light-primary text-primary">{{$product->country->name}}</span>
                                        </div>
                                        <div class="mb-1">
                                            <span class="badge rounded-pill font-weight-medium bg-light-primary text-warning">{{$product->country_team->name}}</span>
                                        </div>
                                    @endif
                                </td>
